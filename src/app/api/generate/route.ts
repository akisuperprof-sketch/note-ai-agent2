...
4. **【序章：読者を引き込むリード文】**:
   - 目次の前に、読者が「これは自分のことだ」「最後まで読まなければ」と確信するためのプロローグを執筆してください。
   - 共感、悩みへの寄り添い、そしてこの記事を読むことで得られるドラマチックな変化を期待させる内容にしてください。
5. **【目次】**:
   - 序章の直後に、全体の構成を伝えるための「目次」を作成してください。
   - 形式は「目次」という見出しの下に、1. 2. 3. と番号を振ったリスト形式で出力してください。
6. **本文の執筆**: 
   - 計画した構成案に基づき、各セクションを1つずつ深掘りして執筆してください。
   - **記事本文の中に「記事の構成」や「構成案」などのメタ的な言葉は絶対に含めないでください。**
