...
1. **【計画・構成案】（内部用）**: 
   ...
   - **【最優先ルール】**: もし「メインテーマ」や「目次の構成・補足」の中に、具体的な箇条書きや章立て（例: ・〜について、1章:〜、など）が含まれている場合は、**その構造を絶対的な骨子として採用してください。** AI独自の判断で勝手に構成を変えないでください。
   - **目標文字数（${targetLength}文字）を達成するために、必ず${minSectionCount}個以上のH2見出しを立ててください。**（ユーザーからの指定が少ない場合は、指定された内容を膨らませてセクション数を確保してください）
...
3. **# 記事タイトル**: 
   ...
   - **スマホでの視認性を考慮し、必ず「35文字以内」で出力してください（2行で美しく収まる長さ）。**
   - 長くなりそうな場合は、サブタイトル（〜...〜）などは省き、本質的な言葉だけに絞ってください。
4. **【序章：リード文】**:
